...
function OrganicBranch({ parent, child, parentStatus }) {
...
function SkillBud({ skill, onClick, selected, index }) {
...
function PracticeSheet({ skill, onClose, onLogXp }) {
  const config = STATUS_CONFIG[skill.status];
  const isLocked = skill.status === "locked";
  const [logAmount, setLogAmount] = useState(null);
...
export default function SkillTreeV2() {
  ...
  const handleLogXp = useCallback((skillId, amount) => {
    setTrees((prev) => {
  ...
  return (
    <div style={{
      ...
      <link href="https://fonts.googleapis.com/css2?family=Playfair+Display:ital,wght@0,400;0,500;0,600;0,700;1,400&family=DM+Sans:wght@400;500;600&display=swap" rel="stylesheet" />
      <style>{`
